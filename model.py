...
def recognize_faces_live_Single(threshold=0.6, frame=None):
...
def recognize_faces_live_Multi(threshold=0.6, frame=None):
    registered = load_registered_embeddings()
...
def video_generator(mode):
    cap = cv2.VideoCapture(0)  # 0 is default webcam
    if not cap.isOpened():
        print("[!] Cannot access webcam")
        return
    cv2.namedWindow('Face Recongnition System', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Face Recongnition System', 800, 600)
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
        if mode == 'single':
            returned_frame = recognize_faces_live_Single(frame=frame)
        elif mode == 'multi':
            returned_frame = recognize_faces_live_Multi(frame=frame)
        else:
            print("[!] Invalid mode. Use 'single' or 'multi'.")
            break
        ret, jpeg = cv2.imencode('.jpg', returned_frame)
        if not ret:
            continue
        yield jpeg.tobytes()